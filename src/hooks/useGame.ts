...
export const useGame = (size: number) => {
  const getTilesMatrix = useCallback(
    (tiles: TileMeta[]): TileMeta[][] => {
  ...
  const moveState = useCallback(
    (state: TileMeta[], direction: Direction): MoveStateReturn => {
      ...
      const [pointerStart, pointerStep] =
        direction === 'DOWN' || direction === 'RIGHT' ? fromEndToStart : fromStartToEnd;
  ...
  return { moveState, generateRandomTile, hasPossibleMoves };
};
